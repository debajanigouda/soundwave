...
export default function Sidebar({ currentPage, setCurrentPage, playlists, likedCount }) {
  ...
  const navItems = [
    ...
    { id: "liked",     label: "Liked",        icon: <HeartIcon />, badge: likedCount },
  ];
  ...
  if (isMobile) {
    return (
      <>
        {/* Mobile Header */}
        <div style={{ background: "#12121a", borderBottom: "1px solid #2a2a45", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", gridColumn: "1", zIndex: 100 }}>
          ...
          <button onClick={() => setShowSidebar(!showSidebar)}
            style={{ background: "none", border: "none", color: "#a0a0c0", cursor: "pointer", fontSize: 24 }}>☰</button>
        </div>
        ...
        {showSidebar && (
          <div style={{ position: "fixed", inset: 0, zIndex: 999 }}>
            ...
            {/* Drawer */}
            <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 280, background: "#12121a", borderRight: "1px solid #2a2a45", display: "flex", flexDirection: "column", padding: "20px 12px", overflowY: "auto" }}>
              {/* Logo */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, padding: "0 12px" }}>
                ...
                <button onClick={() => setShowSidebar(false)}
                  style={{ background: "none", border: "none", color: "#606080", cursor: "pointer", fontSize: 20 }}>✕</button>
              </div>
              ...
              {[...navItems, { id: "downloads", label: "Downloads", icon: <DownloadIcon /> }].map(item => (
                <button key={item.id} onClick={() => { setCurrentPage(item.id); setShowSidebar(false); }}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 12px", borderRadius: 10, cursor: "pointer", color: currentPage === item.id ? "#6c63ff" : "#a0a0c0", background: currentPage === item.id ? "rgba(108,99,255,.15)" : "none", border: "none", width: "100%", textAlign: "left", fontSize: 14, fontWeight: 500, fontFamily: "inherit", marginBottom: 2 }}>
                  {item.icon}
                  {item.label}
        ...
        {/* Mobile Bottom Navigation */}
        <div style={{ position: "fixed", bottom: 80, left: 0, right: 0, background: "#12121a", borderTop: "1px solid #2a2a45", display: "flex", justifyContent: "space-around", padding: "8px 0", zIndex: 100 }}>
          {navItems.map(item => (
            <button key={item.id} onClick={() => setCurrentPage(item.id)}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, background: "none", border: "none", cursor: "pointer", color: currentPage === item.id ? "#6c63ff" : "#606080", padding: "4px 16px", position: "relative" }}>
              {item.icon}
              <span style={{ fontSize: 10, fontWeight: 500, fontFamily: "inherit" }}>{item.label}</span>
              {item.badge !== undefined && item.badge > 0 && (
                <span style={{ position: "absolute", top: 0, right: 8, background: "#6c63ff", color: "#fff", fontSize: 9, padding: "1px 5px", borderRadius: 20, fontWeight: 700 }}>{item.badge}</span>
              )}
            </button>
          ))}
        </div>
...
function HomeIcon()     { return <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg> }
function SearchIcon()   { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg> }
function LibraryIcon()  { return <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/></svg> }
function HeartIcon()    { return <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg> }
function DownloadIcon() { return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> }