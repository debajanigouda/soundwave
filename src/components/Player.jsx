...
export default function Player({ currentSong, isPlaying, isBuffering, togglePlay, nextSong, prevSong, progress, duration, seekTo, volume, setVolume, isMuted, setIsMuted, isShuffle, setIsShuffle, isRepeat, setIsRepeat, likedSongs, toggleLike, isMobile }) {
  ...
  if (isMobile) {
    return (
      <div style={{ background: "#12121a", borderTop: "1px solid #2a2a45", padding: "8px 16px", flexShrink: 0 }}>
        {/* Progress bar */}
        <div onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); seekTo((e.clientX - r.left) / r.width); }}
          style={{ width: "100%", height: 3, background: "#222236", borderRadius: 4, cursor: "pointer", marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {/* Thumbnail */}
          {currentSong ? (
            <img src={currentSong.thumbnail} alt="" style={{ width: 44, height: 44, borderRadius: 8, objectFit: "cover", flexShrink: 0 }}/>
          ) : (
            <div style={{ width: 44, height: 44, borderRadius: 8, background: GRAD, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>♪</div>
          )}
          ...
          <button onClick={() => currentSong && toggleLike(currentSong.id)}
            style={{ background: "none", border: "none", cursor: "pointer", color: currentSong && likedSongs.has(currentSong.id) ? "#ff6b9d" : "#606080", padding: 6, flexShrink: 0 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill={currentSong && likedSongs.has(currentSong.id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
              <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
            </svg>
          </button>

          <button onClick={prevSong} style={{ background: "none", border: "none", cursor: "pointer", color: "#a0a0c0", padding: 6, flexShrink: 0 }}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/></svg>
          </button>

          <button onClick={togglePlay}
            style={{ width: 44, height: 44, borderRadius: "50%", background: "#f0f0ff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            {isBuffering
          ...
          <button onClick={nextSong} style={{ background: "none", border: "none", cursor: "pointer", color: "#a0a0c0", padding: 6, flexShrink: 0 }}>
  ...
  return (
    <footer style={{ background: "#12121a", borderTop: "1px solid #2a2a45", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, flexShrink: 0, height: 100 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, width: 260, flexShrink: 0 }}>
        {currentSong ? (
          <img src={currentSong.thumbnail} alt={currentSong.title} style={{ width: 54, height: 54, borderRadius: 10, objectFit: "cover", flexShrink: 0 }}/>
        ) : (
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, maxWidth: 600 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <CtrlBtn onClick={() => setIsShuffle(s => !s)} active={isShuffle}>
          ...
          <CtrlBtn onClick={nextSong}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>
          </CtrlBtn>
          <CtrlBtn onClick={() => setIsRepeat(r => !r)} active={isRepeat}>
        ...
        <div style={{ width: "100%", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div onClick={(e) => { const r = e.currentTarget.getBoundingClientRect(); seekTo((e.clientX - r.left) / r.width); }}
            style={{ flex: 1, height: 4, background: "#222236", borderRadius: 4, cursor: "pointer" }}
...
function CtrlBtn({ onClick, active, children }) {
  return (
    <button onClick={onClick}
      style={{ width: 36, height: 36, borderRadius: "50%", background: "none", border: "none", color: active ? "#6c63ff" : "#a0a0c0", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
      {children}
    </button>
  );
}